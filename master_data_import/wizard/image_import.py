...
addons_path = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'static/src/img/'))


class ProductMultiImageImport(models.TransientModel):
    _name = 'product.multi.image.import'
    ...
    def _write_bounced_images(self, file_head, bounced_detail, context):
        ...
        try:
            ...
            with open(file_head+"/"+fname, 'w', newline='') as csvfile:
                fl = csv.writer(csvfile, delimiter=' ', quotechar=' ', quoting=csv.QUOTE_MINIMAL)
    ...
    def confirm_import(self):
        ...
        for data in images_data[0]:
            ...
            if product_id:
                if not product_id.product_tmpl_id.product_template_image_ids.filtered(lambda a: a.name == data['filename']):
                    # data_img = tools.image_get_resized_images(data['image'], return_big=True)
                    image = data['image']
                    product_id.product_tmpl_id.product_template_image_ids = [(0, 0, {'name': data['filename'], 'image_1920': image})]
                else:
                    bounce_imgs.append(['Image name already exist: ' + str(data['filename'])])
            else:
        ...
        if bounce_imgs:
            ...
            f.close()
            ctx.update({'default_is_bounced': True, 'default_file': file['file_path'].split("/")[-1]})
        ...
        return {
                ...
                'view_type': 'form',
                'view_mode': 'form',
                'res_model': 'multi.output.message',
                'type': 'ir.actions.act_window',
                'target': 'new'
                }


class MultiOutputMessage(models.TransientModel):
    _name = 'multi.output.message'
