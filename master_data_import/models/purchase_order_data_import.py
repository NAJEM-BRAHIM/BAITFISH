...
class PurchaseOrderDataImport(models.Model):
    _name = 'po.data.import'
    _inherit = ['mail.thread']
    _description = 'Purchase Order Data Import'
    ...
    source_dest_ids = fields.One2many('source.destination.object', 'po_data_import_id', string="Source Destination")
            ...
            if isinstance(process_val, list):
                if process_key == 'partner_id':
                    ...
                    for line_val in process_val:
                        line_data = models.execute_kw(database, uid, password, 'purchase.order.line', 'search_read', [[('id', '=', line_val)]], {'fields': ['id', 'name', 'product_uom', 'date_planned', 'product_qty', 'order_id', 'price_unit', 'product_id', 'taxes_id']})
    ...
    def prepare_order_line_dictionary(self, line_data, models, database, uid, password, model_write_id):
        ...
        for line in line_data:
            order = self.env['purchase.order'].search([('name', '=', line['order_id'][1].split('(')[0].strip())])
            ...
            if len(line['taxes_id']) == 1:
                line_taxes = models.execute_kw(database, uid, password, 'account.tax', 'search_read', [[('id', '=', line['taxes_id'])]], {'fields': ['id', 'name']})[0]['name']
